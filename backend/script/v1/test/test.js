...
describe('Testing the endpoints', () => {
  it('It should create a user', done => {
    ...
    chai.request(app).post('/api/v1/users/').set('Accept', 'application/json').send(user).end((err, res) => {
  ...
  it('It should not get a particular user with invalid id', done => {
    const userId = 8888;
    chai.request(app).get('/api/v1/users/${userId}').set('Accept', 'application/json').end((err, res) => {
      expect(res.status).to.deep.equal(404); //expect(res.body).to.eql('Cannot find the user with the id ${userId}');
  ...
  it('It should not get specific user with non-numeric id', done => {
    const userId = 'aaa';
    chai.request(app).get('/api/v1/users/${userId}').set('Accept', 'application/json').end((err, res) => {
      expect(res.status).to.deep.equal(404); //expect(res.body).to.eql(`Please enter a numeric value`);
  ...
  it('It should update a user', done => {
    ...
    chai.request(app).put('/api/v1/users/${userId}').set('Accept', 'application/json').send(updateUser).end((err, res) => {
      expect(res.status).to.equal(200);
  ...
  it('It should not update a user with non numeric id', done => {
    ...
    chai.request(app).put('/api/v1/users/${userId}').set('Accept', 'application/json').send(updateUser).end((err, res) => {
      expect(res.status).to.equal(400);
      res.body.should.have.property('message').eql('Input a valid numeric value');
      done();
    });
  });
  ...
  it('It should not delete a user with non-numeric id', done => {
    ...
    chai.request(app).delete(`/api/v1/users/${userId}`).set('Accept', 'application/json').end((err, res) => {
      expect(res.status).to.equal(404);
      res.body.should.have.property('message').eql(`Provide a numeric value`);
      done();
    });
  });
  ...
  it('It should fail to create an article ', done => {
    const data = {};
    chai.request(app).post('/api/v1/articles').send(data).set('Accept', 'application/json').end((request, response) => {
  ...
  it('It should fail to create an article due database issues ', done => {
    chai.request(app).post('/api/v1/articles').send(data).set('Accept', 'application/json').end((request, response) => {
      response.body.should.have.property('status').equal(501);
      done();
    });
  });
  it('It should create an article', done => {
    ...
    chai.request(app).post('/api/v2/articles').send(data).set('Accept', 'application/json').end((request, response) => {
      ...
      response.body.data.should.have.property('user_id').equal(data.user_id);
      done();
    });
  });
  it('It should not find article', done => {
    const articleID = -1;
    chai.request(app).get(`/api/v1/articles/${articleID}`).set('Accept', 'application/json').end((request, response) => {
  ...
  it('should get feeds', done => {
    chai.request(server).get('/api/v1/feeds').set('Accept', 'application/json').end((request, response) => {
      response.body.should.have.property('status').equal(200);
  ...
  it('It should delete article', done => {
    const articleID = 1;
    chai.request(app).delete(`/api/v1/articles/${articleID}`).set('Accept', 'application/json').end((request, response) => {
  ...
  it('It should fail to add comment', done => {
    const comment = '';
    const articleId = 4;
    chai.request(app).post(`/api/v1/articles/${articleId}/comments`).set('Accept', 'application/json').send({
      comment
  ...
  it('It should fail to update article', done => {
    ...
    chai.request(app).put(`/api/v1/articles/${articleId}`).set('Accept', 'application/json').send(data).end((request, response) => {
      response.body.should.have.property('status').equal(404);
      response.body.should.have.property('message').equal('Article not found !');
    });
    done();
  ...
  it('It should fail to update article', done => {
    const articleId = 1;
    chai.request(app).patch(`/api/v1/articles/${articleId}`).set('Accept', 'application/json').send({}).end((request, response) => {
      response.body.should.have.property('status').equal(422);
    });
    done();
  });
  it('It should edit article', done => {
    ...
    chai.request(app).patch(`/api/v1/articles/${articleId}`).set('Accept', 'application/json').send(data).end((request, response) => {
      response.body.should.have.property('status').equal(200);
      response.body.should.have.property('message').equal('Article successfully edited');
      response.body.should.have.property('data');
      response.body.data.should.be.an('Object');
      response.body.data.should.have.property('title').equal(data.title);
      response.body.data.should.have.property('article').equal(data.article);
  ...
  it('It should not find article, wrong tag', () => {
    const tagId = 100;
    chai.request(app).get(`/api/v1/feeds/${tagId}/tags`).set('Accept', 'application/json').end((request, response) => {
      response.body.should.have.property('status').equal(404);
      response.body.should.have.property('message').equal('No articles found !');
    });
  });
  ...
  it('It should not find articles by author', () => {
    const authorId = 0;
    chai.request(app).get(`/api/v1/author/articles/${authorId}`).set('Accept', 'application/json').end((request, response) => {
      response.body.should.have.property('status').equal(404);
      response.body.should.have.property('message').equal('No articles found !');
    });
  });
  ...
  it('It should create an gif', done => {
    ...
    chai.requests(app).post('/api/v1/gifs').set('Accept', 'application/json').send(user).end((err, res) => {
  ...
  it('It should not create a gif with incomplete form fields', done => {
    ...
    chai.request(app).post('/api/v1/gifs').set('Accept', 'application/json').send(gifs).end((err, res) => {
      expect(res.status).to.equal(400);
      done();
    });
  });
  it('It should get all gifs', done => {
    chai.request(app).get('/api/v1/gifs').set('Accept', 'application/json').end((err, res) => {
      expect(res.status).to.equal(200);
      res.body.data[0].should.have.property('id');
  ...
  it('It should get a particular gif', done => {
    const gifId = 1;
    chai.request(app).get('/api/v1/gifs/${gifId}').set('Accept', 'application/json').end((err, res) => {
      expect(res.status).to.equal(200);
      res.body.data[0].should.have.property('id');
      res.body.data[0].should.have.property('title');
      res.body.data[0].should.have.property('gifUrl');
  ...
  it('It should not get a particular gif with invalid id', done => {
    ...
    chai.request(app).get('/api/v1/gifs/${gifId}').set('Accept', 'application/json').end((err, res) => {
      expect(res.status).to.equal(404).res.body.should.have.property('message').eql('Cannot find gif with the id ${gifId}');
      done();
    });
  });
  it('It should not get specific gif with non-numeric id', done => {
    ...
    chai.request(app).get('/api/v1/gifs/${gifid}').set('Accept', 'application/json').end((err, res) => {
      expect(res.status).to.equal(404);
      res.body.should.have.property('message').eql('Please input a numeric value');
      done();
    });
  });
  it('It should update a gif', done => {
    ...
    chai.request(app).put('/api/v1/gifs/${gifId}').set('Accept', 'application/json').send(updateGif).end((err, res) => {
      expect(res.status).to.equal(200);
  ...
  it('It should not update a gif with invalid id', done => {
    ...
    chai.request(app).put('/api/v1/gifs/${gifId}').set('Accept', 'application/json').send(updateUser).end((err, res) => {
      expect(res.status).to.equal(400);
      res.body.should.have.property('message').eql('Cannot find gif with the id ${gifId}');
      done();
    });
  });
  it('It should not update a gif with non numeric id', done => {
    ...
    chai.request(app).put('/api/v1/gifs/${gifId}').set('Accept', 'application/json').send(updateArticle).end((err, res) => {
      expect(res.status).to.equal(400);
      res.body.should.have.property('message').eql('Input a valid numeric value');
      done();
    });
  ...
  it('It should delete a gif', done => {
    const gifId = 1;
    chai.request(app).delete('/api/v1/gifs/${gifId}').set('Accept', 'application/json').end((err, res) => {
      expect(res.status).to.equal(200);
      expect(res.body.data).to.include({});
      done();
    });
  ...
  it('It should not delete a gif with invalid id', done => {
    ...
    chai.request(app).delete(`/api/v1/gifs/${gifId}`).set('Accept', 'application/json').end((err, res) => {
      expect(res.status).to.equal(404);
      res.body.should.have.property('message').eql(`Gif with the id ${gifId} cannot be found`);
      done();
    });
  });
